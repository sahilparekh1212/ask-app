...
| ✅ Done | **Voice chat** | Ask by voice and hear the answer back — a mic button dictates your question into the composer (browser Web Speech API), and a speaker control reads a reply aloud in a natural neural voice via a server-side Google Cloud Text-to-Speech proxy (Chirp3-HD, the voice behind Google Assistant). A third **hands-free "voice only"** control turns it into a continuous eyes-free conversation: it listens, answers aloud in a short spoken reply, then listens again until you stop it. It gracefully falls back to the browser's built-in voice when no TTS key is configured or the browser lacks support. |
