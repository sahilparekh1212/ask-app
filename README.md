...
| ✅ Done | **Voice chat** | Ask by voice and hear the answer back — a mic button dictates your question into the composer and a speaker control reads a reply aloud, both via the browser's built-in Web Speech API (no backend, no keys, and gracefully hidden where the browser doesn't support it). |
