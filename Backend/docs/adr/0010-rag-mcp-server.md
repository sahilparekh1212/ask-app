...
### Addendum 3 — indexing the synthetic security-master reference data

The corpus was extended once more to include the **security-master reference data** so the
assistant and MCP `search_knowledge` can answer questions about the dataset (what instruments
exist, their asset class / currency / price), not only about the repo's docs and code.

Two things distinguish this from the earlier corpus additions and are worth recording against
Decision 3 and Decision 5:

- **Query-derived, not build-time-bundled.** Unlike the docs/code corpus (fixed in the jar), the
  reference chunks are read from the database at index time (`ReferenceDataChunker`), so they
  reflect the *current* rows — including the daily incremental batch, which triggers a re-index.
  The chunking is chosen for cheap incremental hashing: a single count-free overview chunk plus
  fixed-size windows of consecutive instruments. Because the data is append-only, every already-
  full window keeps its content hash across re-indexes, so only the trailing/new window re-embeds.
- **Boundary check — still "public, non-PII data only".** Decision 5's revisit trigger is *any
  non-public data* entering the corpus. The reference data is **synthetic** (generated by
  `RefDataGenerator`, no real securities, no user data), so it stays on the public side of that
  line and the endpoint remains `permitAll`. The invariant that actually matters — **audit rows
  and user data are never indexed** — is unchanged (Decision 4, point 1 still holds). Were real
  or licensed reference data ever loaded, this is exactly the trigger to move `/mcp` behind the
  JWT gate and scope the tools by role. Toggle with `rag.reference-data.enabled`.
