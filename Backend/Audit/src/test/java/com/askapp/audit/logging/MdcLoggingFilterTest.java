package com.askapp.audit.logging;
...
class MdcLoggingFilterTest {
	...
	void honoursSuppliedRequestIdAndEchoesItBack() throws Exception {
		HttpServletRequest request = request("req-abc", null, "/api/audit-logs/search");
		...
		// Cleared after the chain completes.
	...
	void extractsSubFromBearerToken() throws Exception {
		HttpServletRequest request = request(null, "Bearer " + jwt("{\"sub\":\"user-123\"}"), "/api/audit-logs");
	...
	void summarisesUriToLastTwoSegments() throws Exception {
		assertThat(runAndCaptureMdc(request(null, null, "/api/audit-logs/search"), mock(HttpServletResponse.class)))
			.containsEntry("urlParts", "audit-logs/search");
