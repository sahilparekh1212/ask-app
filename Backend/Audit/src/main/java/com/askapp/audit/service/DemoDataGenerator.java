 ...
 * Rag/SEARCH, Mcp/TOOL_CALL) — so a demo-populated dashboard tells the same story real traffic
 * would, not a generic e-commerce one. Details mirror the real events' non-PII {@code key=value}
 * shape (model, latency, counts); no message or query text.
 *
