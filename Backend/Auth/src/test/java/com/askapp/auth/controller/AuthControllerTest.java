...
import com.askapp.auth.model.LoginRequest;
import com.askapp.auth.model.RefreshRequest;
...
class AuthControllerTest {
	...
	void login_issuesTokensAndPublishesALoginEvent() {
		...
		when(tokenService.generateTokens("demo-user", "demo@askapp.dev", "Demo User", Roles.ROLE_USER))
			.thenReturn(tokens);
	...
	void login_mintsTheRequestedRoleWhenGiven() {
		TokenResponse tokens = new TokenResponse("access", "refresh", 1800L);
		when(tokenService.generateTokens("demo-user", "demo@askapp.dev", "Demo User", Roles.ROLE_ADMIN))
			.thenReturn(tokens);

		ResponseEntity<TokenResponse> response = controller.login(new LoginRequest("demo", "demo", Roles.ROLE_ADMIN));

		assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
		verify(tokenService).generateTokens("demo-user", "demo@askapp.dev", "Demo User", Roles.ROLE_ADMIN);
	}
