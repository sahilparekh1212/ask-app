...
class RateLimitInterceptorTest {
	...
	void preHandle_registersUnderAnAuthenticatedPerUserPerEndpointKey() {
		...
		ActiveRequest mine = new ActiveRequest("alice|GET|/api/auth", Thread.currentThread());
		...
		assertThat(key.getValue()).isEqualTo("alice|GET|/api/auth");
	...
	void preHandle_fallsBackToAnonymousWhenUnauthenticated() {
		...
		ArgumentCaptor<String> key = ArgumentCaptor.forClass(String.class);
		verify(registry).register(key.capture());
		assertThat(key.getValue()).isEqualTo("anonymous|POST|/api/auth");
	}
